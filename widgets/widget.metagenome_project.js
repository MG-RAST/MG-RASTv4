(function() {
    ...
    widget.display = function(params) {
        ...
        if (widget.id) {
            ...
            if (project.status == 'private' || canEdit) {
                html += "<h3 class='alert alert-info'><button class='btn' style='margin-right: 15px; position: relative; bottom: 3px;' onclick='window.location=\"mgmain.html?mgpage=share&project=" + Retina.cgiParam('project') + "\";' title='edit / share project'><i class='icon icon-share'></i> study editor</button>" + (project.status == 'public' ? "" : "Private Study: ") + project.name + "</h3>";
            } else {
            ...
            Retina.Renderer.create("table", {
                ...
                data: {
                    data: rows,
                    header: ["MG-RAST ID", "name", "bp count", "seq. count", "biome", "feature", "material", "sample", "library", "ENA", "location", "country", "coordinates", "type", "method", "download"]
                }
            }).render();
    ...
    widget.metadataDownload = function() {
        ...
        if (stm.authHeader && stm.authHeader.Authorization) {
            url += "&auth=" + stm.authHeader.Authorization;
        }
        window.location = url;
    };
