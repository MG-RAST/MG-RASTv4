(function () {
	...
	if (! widget.dataLoaded) {
		...
		jQuery.getJSON("data/metagenome_overview.flow.json").then(function(d) {
			...
			success: function (data) {
				...
				images.push({"url": RetinaConfig.shock_url+"/node/"+data.data[i].id+"?download", "name": data.data[i].file.name });
			    }
	...
	var notebook = Retina.Renderer.create('notebook', { target: container, showTOC: false, tocTarget: sidebar, editTarget: sidebar, flow: stm.DataStore.flows.metagenome_overview, dataContainer: stm.DataStore.metagenome[widget.id], editMode: true, images: widget.images }).render();	
    };  
