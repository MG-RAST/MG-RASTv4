...
<dt>Public Access</dt><dd>You can make your data public once you have provided the minimal amount of metadata. Public data can be viewed by any registered or anonymous user, obviously public access is readonly. Public access cannot be revoked.</dd>\
	...
	if (found !== null && stm.DataStore.project[found].hasOwnProperty('permissions')) {
	    ...
	    // project permissions
	    var po = stm.DataStore.project[found];
	    var isPublic = po.status == 'public' ? true : false;
	    var mgid2name = {};
	    for (var i=0; i<po.metagenomes.length; i++) {
		mgid2name[po.metagenomes[i].metagenome_id] = po.metagenomes[i].name;
	    }
	    ...
	    html += "</table><h4>metagenomes</h4><table class='table table-condensed'><tr><td>name</td><td>user / group</td><td>edit</td><td>view<img src='Retina/images/"+(widget.metagenomeRightsLocked ? "lock" : "unlocked")+".png' style='cursor: pointer; width: 16px; float: right;' title='click to "+(widget.metagenomeRightsLocked ? "unlock" : "lock")+"' onclick='Retina.WidgetInstances.metagenome_share[1].unlockMetagenomeRights(this, \""+project+"\", "+(widget.metagenomeRightsLocked ? "false" : "true")+");'></td></tr>";
