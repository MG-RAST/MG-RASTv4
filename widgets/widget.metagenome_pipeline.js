(function () {
    ...
    */
	...
	if (document.getElementById("pageTitle")) {
	    document.getElementById("pageTitle").innerHTML = "job progress";
	}
	...
	if (stm.user) {
	  ...
	  <div>\
		...
		widget.job_table = Retina.Renderer.create("table", {
		    ...
		    minwidths: [1,1,1,1],
		    navigation_url: RetinaConfig['mgrast_api'] + "/pipeline?info.pipeline="+RetinaConfig.pipelines.join("&info.pipeline=")+"&info.user="+widget.userID,
	...
	widget.userID = id;
	widget.job_table.settings.navigation_url = RetinaConfig['mgrast_api'] + "/pipeline?info.pipeline="+RetinaConfig.pipelines.join("&info.pipeline=")+"&info.user="+widget.userID;
	widget.display();
    };
