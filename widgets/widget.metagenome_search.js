...
';
        ...
        // tooltips
        jQuery("#storeresults").popover({
            trigger: "hover",
            html: true,
            content: "<p style='font-weight: normal; line-height: 20px; font-size: 14px; margin-bottom: 0px;'>Saving a search requires you to be logged in.<br><br>You must also choose at least one search parameter.</p>"
        });
        jQuery("#storedresults").popover({
            trigger: "hover",
            html: true,
            content: "<p style='font-weight: normal; line-height: 20px; font-size: 14px; margin-bottom: 0px;'>Store your search query paramters permanently.</p>"
        });
        jQuery("#storedcollections").popover({
            trigger: "hover",
            html: true,
            content: "<p style='font-weight: normal; line-height: 20px; font-size: 14px; margin-bottom: 0px;'>Create a persistent custom list of metagenomes.</p>"
        });
        jQuery("#storecollection").popover({
            trigger: "hover",
            html: true,
            content: "<p style='font-weight: normal; line-height: 20px; font-size: 14px; margin-bottom: 0px;'>Create a new selection of metagenomes.</p>"
        });
        ...
        for (var i = 0; i < keylist.length; i++) {
            ...
            for (var h = 0; h < keylist[i].items.length; h++) {
                if (keylist[i].items[h].value == "created_on") {
                    continue;
                }
    ...
    */
    widget.tableFilter = function(event, blur) {
        ...
        if (event.keyCode == 13 || event.keyCode == 27) {
            ...
            if (event.keyCode == 13) {
                widget.refineSearch('add', {
                    name: widget.fieldnames[inp.getAttribute('field')],
                    key: widget.fields[inp.getAttribute('field')],
                    val: inp.value
                });
            }
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            for (var h = 0; h < fields.length; h++) {
                ...
                if (cell && fields[h] == "created_on") {
                    ...
                    cell = "<div style='max-width: 300px;'><a href='?mgpage=project&project=" + (!data[rows[i][0]]["public"] ? Retina.idmap(data[rows[i][0]]["project_id"]) : data[rows[i][0]]["project_id"]) + "' rel='nofollow' target=_blank>" + data[rows[i][0]]["project_name"] + "</a></div>";
    ...
    widget.queryAPI = function(more, howmany) {
        ...
        jQuery.ajax({
            ...
            success: function(data) {
                ...
                var after_match = /after=(.+?)(&|$)/.exec(data.next);
                widget.lastitem = (after_match == null) ? null : after_match[1];
